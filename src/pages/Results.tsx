...
const Results = () => {
  ...
  const calculateMetrics = () => {
    ...
    const duringInteractionTime =
      (survey.lastInteractionTime || survey.firstInteractionTime) -
  ...
  return (
    <div className="min-h-screen relative bg-secondary">
      ...
      {/* Survey Page Replica */}
      <div className="relative min-h-screen flex flex-col items-center justify-center p-8 bg-secondary animate-fade-in">
        <div className="max-w-2xl w-full space-y-8">
          ...
          <RadioGroup className="space-y-4" value={survey.selectedAnswer || ""}>
            {ANSWERS.map((answer) => (
              ...
              Object.entries(metrics).map(([phase, data]) => (
                <div key={phase} className="bg-gray-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: phase === "pre" ? "#222222" : phase === "during" ? "#888888" : "#AAADB0" }} />
                    <h3 className="text-sm font-medium text-gray-900 capitalize">{phase} Interaction</h3>
                  </div>
                  <p className="text-2xl font-bold text-gray-700">{data.percentage}%</p>
                </div>
              ))}

            <Button onClick={() => navigate("/")}>New Survey</Button>
          </div>
        </div>
      </div>
    </div>
